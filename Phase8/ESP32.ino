...
<p id="status">Connecting to control socket...</p>
...
function connectSocket() {
  ...
  socket.onmessage = event => {
    if (event.data.startsWith('STATE,')) {
      const state = event.data.split(',');
      activeTheme = Number(state[1]);
      markActive(activeTheme);
      document.getElementById('speed').value = state[2];
      document.getElementById('speedLbl').innerText = state[2];
      document.getElementById('brightness').value = state[3];
      document.getElementById('brightLbl').innerText = state[3];
      return;
    }
    if (event.data.startsWith('OK')) {
      const latency = pendingSentAt ? Math.max(0, Math.round(performance.now() - pendingSentAt)) : 0;
      pendingSentAt = 0;
      setStatus('Applied' + (latency ? ' · ' + latency + ' ms' : ''));
      return;
    }
...
function send(command) {
  if (!socket || socket.readyState !== WebSocket.OPEN) {
    setStatus('Control socket is not connected');
    return false;
  }
...
void sendSocketState(uint8_t client) {
  ...
  char response[32];
  snprintf(response, sizeof(response), "STATE,%u,%u,%u", theme, speed, brightness);
  webSocket.sendTXT(client, response);
}
...
void applyWebSocketCommand(uint8_t client, const uint8_t* payload, size_t length) {
    ...
    if (separator != nullptr) {
      *separator++ = '\0';
      ...
      if (parseInteger(args, 0, 13, mode) && isSupportedTheme(mode) &&
        ...
        portENTER_CRITICAL(&stateMux);
        if (activeThemeID != mode || mode == 12 || mode == 13) resetGame = true;
  ...
  if (accepted) webSocket.sendTXT(client, "OK");
  else sendSocketError(client, "invalid-command");
}
