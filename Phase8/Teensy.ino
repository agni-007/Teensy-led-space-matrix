...
bool isSupportedTheme(int theme) {
  return (theme >= 0 && theme <= 4) || (theme >= 7 && theme <= 9) ||
...
void readSerial() {
  while (SERIAL_LINK.available()) {
    ...
    if (c == ',' || c == '>') {
      if (currentToken.length() > 0 && cmdValueIndex < 8)
...
void applyCommand() {
  switch (packetType) {
    case 'A':
      if (cmdValueIndex >= 1 && isSupportedTheme(cmdValues[0])) {
        activeTheme = (ThemeID)cmdValues[0];
        themeClock  = 0;
    ...
    case 'S':
      if (cmdValueIndex >= 3) {
        fgR = constrain(cmdValues[0], 0, 255);
        fgG = constrain(cmdValues[1], 0, 255);
        fgB = constrain(cmdValues[2], 0, 255);
      }
      break;

    case 'B':
      if (cmdValueIndex >= 1) {
        bright = constrain(cmdValues[0], 0, MAX_SAFE_BRIGHTNESS);
        leds.setBrightness(bright);
      }
      break;
    ...
    case 'F': 
      if (activeTheme == THEME_PACMAN && cmdValueIndex >= 1) {
        pacmanPos = (float)cmdValues[0];
      } else if (activeTheme == THEME_DINO && cmdValueIndex >= 8) {
        dinoState    = cmdValues[0];
        dinoY        = (float)cmdValues[1];
        isJumping    = (cmdValues[2] == 1);
        cactusPos[0] = (float)cmdValues[3];
        cactusPos[1] = (float)cmdValues[4];
        cactusPos[2] = (float)cmdValues[5];
        dinoScore    = cmdValues[6];
        dinoLives    = cmdValues[7];
      }
      break;
  }
}
